...
opt = {save = run, optimization = run, learningRate =  1e-3,
 ...
 batchsize=10, threads=8, epochs = 50}
...
model:add(nn.View(64*3*3))                    -- reshapes from a 3D tensor of 16x5x5 into 1D tensor of 16*5*5
model:add(nn.Linear(64*3*3, 64))             -- fully connected layer (matrix multiplication between input and weights)
...
function train(dataset)
   ...
   for t = 1,dataset:size(),opt.batchsize do
      ...
      elseif opt.optimization == 'ADA' then
         config = config or {learningRate = opt.learningRate,
                             weightDecay = opt.weightDecay,
                             momentum = opt.momentum,
                             learningRateDecay = 5e-7}
   ...
   print("Dave trainAccuracy: " .. trainAccuracy)
   confusion:zero()
